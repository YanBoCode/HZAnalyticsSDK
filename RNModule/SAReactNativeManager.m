...
//  [HZAnalyticsSDKManager sharedInstance]
//    if (![[HZAnalyticsSDKManager sharedInstance] isAutoTrackEnabled]) {
//        return;
//    }
//    // 忽略 $AppClick 事件
//    if ([[HZAnalyticsSDKManager sharedInstance] isAutoTrackEventTypeIgnored:SensorsAnalyticsEventTypeAppClick]) {
//        return;
//    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[HZAnalyticsSDKManager sharedInstance] trackViewAppClick:view withProperties:newProps];
    });
}
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        ...
        [[HZAnalyticsSDKManager sharedInstance] trackViewScreen:url withProperties:properties];
...
+ (void)configureSDKWithSettings:(NSDictionary *)settings {
    ...
    HZConfigOptions *options = [[HZConfigOptions alloc] initWithServerURL:serverURL launchOptions:nil];
...
//    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[HZAnalyticsSDKManager sharedInstance] startWithOptions:options appId:[NSString stringWithFormat:@"%@",settings[@"app_id"]] secretKey:[NSString stringWithFormat:@"%@",settings[@"aes_key"]]];
